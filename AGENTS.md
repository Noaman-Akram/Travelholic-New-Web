...
# AGENTS.md — Travelholic Website

> Project memory for Codex. Read first when entering a new session.
...
- [x] **Phase 0** — Bootstrap: Node installed locally, AGENTS.md created
...
- [x] **Phase 4** — Polish, SEO, deploy prep shipped. JSON-LD on all conversion-relevant pages (Organization + WebSite + FAQPage on home; LodgingBusiness + BreadcrumbList + FAQPage on home detail; BreadcrumbList on destination detail; BlogPosting + BreadcrumbList on story detail; LocalBusiness + FAQPage on contact). Dynamic `app/sitemap.ts` (~80 entries, both locales, hreflang per URL incl. x-default). Dynamic `app/robots.ts`. Per-page OG images (1200×630, nodejs runtime) for default + home detail + destination detail + story detail — brand-styled. Cookie consent banner (`TH_CONSENT` cookie) gates GA + Meta Pixel via `<Analytics>` — no third-party scripts inject until "Accept all". A11y: `aria-live` on testimonials carousel + contact form result. Bumped Next 15.1.6 → 15.1.12 (CVE-2025-66478 patched). README.md complete. Build/lint/typecheck green. Vercel deploy is a manual step — see README §"Deploy to Vercel".
- [x] **Phase 5** — Hostify integration (Tier 2: live listings, lead-only bookings). Hostify PMS at `api-rms.hostify.com` with `x-api-key` auth header — typed client at `lib/hostify/{types,client,amenityMap,transform,listings}.ts`. 128 raw Hostify listings group by cleaned title into ~35 unique Travelholic Homes (multiple physical units = one Home with N `hostifyIds`). Tag→destination mapping: Louts→lotus, AUC→auc, South Academy→near-cfc, 90st→ninetieth-street, GG Buildings/Villas/Nomads. Data layer split into client-safe (`@/lib/data`) and server-only (`@/lib/data/server`) — async helpers `getAllHomes/getHomeBySlug/getHomesByDestination/getFeaturedHomes` with mock fallback when `HOSTIFY_API_KEY` is unset. Listing page client refactored: `HomesContext` provides homes via React Context to `useHomesFilters`. Home detail "Book direct" CTA deep-links to `book.travelholiceg.com/listing/{hostifyPrimaryId}` with check-in/out/guests prefilled. USD prices converted to EGP via `NEXT_PUBLIC_EGP_PER_USD` (default 50). Sitemap grew to 116 entries. Smoke-tested live: real Hostify CDN images render, deep-link query params correct, AR locale routes the same way.
  - **Tier 3 deferred** (real reservation creation via Hostify `POST /reservations`) — needs a Hostify staging environment + idempotency + price-mismatch guard before it's safe to ship. Booking dialog still opens for mock-only homes; live homes go straight to the Hostify booking subdomain.
...
- [ ] **Rotate the Hostify API key** that was pasted in chat 2026-05-09. Set the new value directly in `.env.local`.
- [ ] All AR strings — native review pre-launch (see `messages/ar.review.md`)
- [ ] Photography — Hostify CDN provides home photos; brand-level imagery (founder, team, partner wall) still picsum placeholders
...
- [ ] `BOOKING_WEBHOOK_URL` — fallback for lead capture; not used in Tier 2 flow (live homes deep-link to book.travelholiceg.com)
- [ ] Confirm "South Academy" tag on Hostify maps to the **Near CFC** destination on our site (currently mapped that way as a guess — Hostify shows 14 listings tagged "South Academy" but user-stated count for "Near CFC" was 3)
- [ ] FX rate: `NEXT_PUBLIC_EGP_PER_USD` is a static value (default 50). Replace with a live FX feed before launch.
